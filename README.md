...
| [設計仕様書（Lv2）](./design.md) | BFF 方式のシステム構成、UML（アクティビティ図・シーケンス図・クラス図）、ER図、API設計、セキュリティ設計 | 基本設計・詳細設計 | [v1.3](./design_v1.3.pdf) |
| [テスト仕様書（Lv2）](./test_spec.md) | 単体（pytest／jest）・結合・機能・ユーザーテストのケース223件。観点×技法で設計し、期待値は人間が決定 | 単体・結合・システム・受入テスト | [v1.2](./test_spec_v1.2.pdf) |
