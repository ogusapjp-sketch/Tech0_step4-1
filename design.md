...
| 対象 | 規則 |
|---|---|
| 同じコード | 直前に受け付けたコードは、カメラの映像から**連続 0.5秒検出されなくなる**（＝一度離した）まで、再び受け付けない |
| 別のコード | すぐ受け付ける（前のコードをかざしたままでもよい。連続スキャン） |
| 手入力 | この規則を通らない。同じ商品コードを続けて追加できる |

- 判定は「そのコードを**最後に検出した時刻**からの経過時間」で行う。0.5秒たつ前に再び検出されたら、そこから数え直す
- 同じ商品を複数個登録するときは、一度カメラから離してからかざし直す（1個ずつ数量が加算される）。数量をまとめて指定する場合は購入リストの数量変更を使う（FR-007）
- 読み取りの周期は 200ms。検出できなかったフレームも「そのコードが検出されなかった」として数える。
  読み取り自体が例外になったフレーム（映像の準備前など）は、離したものとして扱わない
- 実装：`frontend/src/lib/scanGate.ts`（`createScanGate`、`SCAN_RELEASE_MS = 500`）と `BarcodeScanner`
