...
| v1.8 | 2026-09-22 | 重複防止の判定を確かめられるよう、開発環境（`APP_ENV=development`）のときだけカメラ枠の下に読み取りの状態を表示し、受け付けるたびにコンソールへ出力する（6.4）。本番では表示も出力もしない |
| v1.9 | 2026-09-22 | 1フレームに複数のバーコードが写る場合（メニュー早見表）の扱いを追加（6.4）。映像の中心に最も近い1件だけを採用し、受付の状態をコードごとに持つ（複数のコードが交互に「別のコード」として受け付けられ、繰り返し追加されたため） |
| v1.10 | 2026-09-22 | 6.4 を補正。フレームで検出したコードは採用の有無に関係なく状態を持ち、未検出のフレーム数を増やすのは写っていないコードだけとする。受付は「まだ受け付けていないコード」または「離したと判定済みのコード」に限る |
...
カメラはかざしている間ずっと同じコードを読み続けるため、読み取るたびに登録すると同じ商品が何件も追加される。
そのため、カメラからの読み取りには次の規則を設ける。手入力（照会 → 追加の2段階）はこの規則の対象外とする。

| 対象 | 規則 |
|---|---|
| 同じコード | 直前に受け付けたコードは、**一度離した**と判断できるまで再び受け付けない。判断の条件は下表の2つを**ともに**満たすこと |
| 別のコード | すぐ受け付ける（前のコードをかざしたままでもよい。連続スキャン） |
| 手入力 | この規則を通らない。同じ商品コードを続けて追加できる |

**1フレームに複数のバーコードが写る場合**

メニュー早見表は複数のカードが並ぶため、1フレームに複数のコードが写る。フレームごとに**1件だけ採用**する。

| 経路 | 採用するもの |
|---|---|
| Barcode Detection API | `boundingBox` の中心が映像の中心に最も近い1件（位置が取れないものは先頭） |
| ZXing | 1フレームに1件しか返さないため、その1件 |

**受付の判定（採用した1件について）**

受付の状態は**コードごと**に持つ（`lastSeenAt`・`misses`・`released`）。

| 状況 | 動作 |
|---|---|
| まだ受け付けたことがないコード | 受け付ける（理由「初出」） |
| 受け付けたことがあり、「離した」と判定済み（`released`） | 受け付ける（理由「離した後の再受付」） |
| それ以外（かざしたまま） | 受け付けない |

**状態の更新（受付の可否とは別に、毎フレーム行う）**

| 対象 | 動作 |
|---|---|
| そのフレームで検出したコード**すべて**（採用されなかったものを含む） | `lastSeenAt` を更新し、`misses` を 0 に戻す |
| そのフレームで検出されなかった既知のコード | `misses` を1増やす |

早見表のように複数のカードが写り続ける場合、写っているコードは「見えている」ものとして扱う。
採用されなかったことを未検出として数えると、中心が入れ替わるたびに「離した」と誤判定される。

**「離した」と判断する条件（2つとも満たすこと）**

| 定数 | 値 | 内容 |
|---|---|---|
| `SCAN_RELEASE_MS` | 1000 | そのコードを最後に検出してからの経過時間が、この値以上であること |
| `SCAN_RELEASE_FRAMES` | 5 | 読み取りを試みたフレームのうち、連続してこの回数以上、そのコードが採用されていないこと |

時間だけを条件にすると、手ぶれや照明でわずかに読み取りが途切れただけでも「離した」と判定され、
かざしたままでも繰り返し追加されてしまう。フレーム数の条件を加えて、読み取りの一時的な途切れと区別する。

- 途中で一度でもそのコードが採用されたら、経過時間・フレーム数とも数え直す
- `misses` が `SCAN_FORGET_FRAMES`（30）に達したコードは状態から削除する（早見表を持ち替えたあとの再スキャンに備える）
- 同じ商品を複数個登録するときは、一度カメラから離してからかざし直す（1個ずつ数量が加算される）。
  数量をまとめて指定する場合は購入リストの数量変更を使う（FR-007）
- 読み取りの周期は 200ms。読めなかったフレームも1フレームとして数える。
  Barcode Detection API・ZXing のどちらの経路でも、読めなかったフレームを空として同じ仕組みに渡す
...
- **開発環境（`APP_ENV=development`）のときだけ**、カメラ枠の下に判定の状態を表示する。
  ...
  管理中のコード数、`misses`、受け付けた回数と直近の理由。あわせて、受け付けるたびに理由・前回の検出からの経過時間・`misses` を
  ブラウザのコンソールへ出力する。`APP_ENV` が未設定・`production` のときは、表示も出力も行わない（設定漏れは本番扱い）
- 実装：`frontend/src/lib/scanGate.ts` の純粋関数 `pickCenterMost`（採用の選択）と `nextScanState`（受付の判定。状態は `Map<コード, {lastSeenAt, misses, released}>`）、
  および `BarcodeScanner`。BarcodeDetector・ZXing のどちらの経路も、この2つの関数を通る
