...
- 重複防止の状態（最後に検出した時刻・検出されなかったフレーム数）は**画面で1つだけ**持つ。
  商品の追加で画面が再描画されても、カメラの部品が作り直されても、状態を引き継ぐ
