...
要件定義書 3.3 の業務フローを、システムの処理として表す。会員ID読み込み（①）は購入確定までのどの時点でも行える（Lv2 2.8）ため、商品登録と並行する分岐として描く。

```mermaid
flowchart TD
    S([勤務開始]) --> L[ログイン<br>担当者ID・パスワード]
    L --> L1{認証OK?}
    L1 -- No --> L2[エラー表示] --> L
    L1 -- Yes --> R([レジ画面<br>次の客を待つ])
    ...
    R --> PI[商品コードを手入力] --> P
    M --> M1{会員存在?}
    M1 -- No --> M2[「該当会員なし」表示] --> R
    M1 -- Yes --> M3[会員情報を表示] --> E

    P --> P1{マスタに存在?}
    P1 -- No --> P2[「商品がマスタ未登録です」表示] --> R
    P1 -- Yes --> P3{購入リストに<br>同一商品あり?}
    P3 -- Yes --> P4{数量 < 99?}
    P4 -- Yes --> P5[数量 +1]
    P4 -- No --> P6[「上限に達しています」表示] --> R
    P3 -- No --> P7[新規行を追加<br>数量 1]
    P5 --> CL[入力欄をクリア]
    P7 --> CL
    CL --> E

    E[値引きを再評価<br>合計を再計算] --> R
    R --> ED[行を選択して<br>削除／数量変更 1〜99] --> E

    R --> C[購入ボタン]
    C --> C1{購入リストが空?}
    C1 -- Yes --> R
    C1 -- No --> C2[バックエンドで再計算し<br>フロントの金額と照合]
    C2 --> C3{一致?}
    C3 -- No --> C4[エラー表示<br>確定しない] --> R
    C3 -- Yes --> C5[取引を保存] --> C6[税込合計を<br>ポップアップ表示]
    C6 --> C7[ポップアップを閉じる<br>購入リスト・会員情報をクリア] --> R
```
...
classDiagram
    ...
    class PricingService {
        +calculate(items, member, campaigns, tax_rate_bp) Totals
        +apply_discount(item, campaigns, now) int
    }
