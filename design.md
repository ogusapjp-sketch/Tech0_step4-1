...
| v1.7 | 2026-09-22 | スキャンの重複防止（6.4）の解除条件を見直し。時間だけでは解除せず、「最後の検出から 1.0秒以上」かつ「連続5フレーム検出されない」の両方を満たしたときに再受付とする（読み取りの一時的な途切れで解除され、かざしたままで繰り返し追加されたため）。カメラの状態表示に読み取り経路（BarcodeDetector／ZXing）を表示 |
