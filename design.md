...
| v1.5 | 2026-09-22 | 検証環境（Azure）に合わせて MySQL 8.4 に変更（2.2）。ローカルの Docker イメージは `mysql:8.4.11` |
| v1.6 | 2026-09-22 | スキャンの重複防止（6.4）を追加。同じコードは、カメラの映像から連続 0.5秒検出されなくなるまで再び読み取らない（かざしたままで同じ商品が繰り返し追加されるのを防ぐ） |
| v1.7 | 2026-09-22 | スキャンの重複防止（6.4）の解除条件を見直し。時間だけでは解除せず、「最後の検出から 1.0秒以上」かつ「連続5フレーム検出されない」の両方を満たしたときに再受付とする（読み取りの一時的な途切れで解除され、かざしたままで繰り返し追加されたため）。カメラの状態表示に読み取り経路（BarcodeDetector／ZXing）を表示。重複防止の状態は画面で1つだけ保持する |
| v1.8 | 2026-09-22 | iPhone 画面表示でのかざし続け・複数写り込みによる重複受付の対策。6.4 を「見えている／採用」の分離、コードごとの状態、中心に最も近い1件の採用、採用の安定性（`STABLE_FRAMES`=3）、解除条件（`RELEASE_MISSES`=12・`RELEASE_MS`=3000）として整理し、状態遷移図を追加。開発環境のみの診断表示を追加。生成AI活用の記録に経緯を追記 |
...
| DB | Azure Database for MySQL Flexible Server（**MySQL 8.4**）。ローカルは Docker の `mysql:8.4.11` | 前提条件（変更不可）。バージョンは Azure の検証環境に合わせる |
...
カメラはかざしている間ずっと同じコードを読み続ける。またメニュー早見表は複数のカードが並ぶため、
1フレームに複数のコードが写る。読み取るたびに登録すると、同じ商品や隣の商品が何件も追加されてしまう。
そこで「**見えている**」ことと「**受け付ける**」ことを分けて扱う。手入力（照会 → 追加の2段階）はこの規則の対象外とする。

#### 6.4.1 フレームごとの処理

| 手順 | 内容 |
|---|---|
| ① 検出 | そのフレームで読み取れたコードをすべて得る（読めなければ0件） |
| ② 採用 | 受付を判定する**1件**を選ぶ。Barcode Detection API は `boundingBox` の中心が映像の中心に最も近いもの（位置が取れないものは先頭）、ZXing は1フレームに1件しか返さないためその1件 |
| ③ 状態の更新 | コードごとの状態（下表）を更新する |
| ④ 受付の判定 | 採用した1件についてだけ行う |

#### 6.4.2 コードごとの状態

状態は `Map<コード, 状態>` で持ち、画面で1つだけ保持する（再描画でも、カメラの部品が作り直されても失わない）。

| 項目 | 内容 | 更新の規則 |
|---|---|---|
| `lastSeenAt` | 最後に検出した時刻 | **検出したコードすべて**（採用されなかったものを含む）で現在時刻に更新 |
| `misses` | 連続して検出されなかったフレーム数 | 検出したコードは 0。検出されなかった既知のコードは +1 |
| `stableFrames` | 連続して採用されたフレーム数 | 採用されたコードは +1。それ以外（検出されたが採用されなかった／検出されなかった）はすべて 0 |
| `released` | 「離した」と判定済みか | 下の条件で true。受け付けたら false |
| `everAccepted` | これまでに受け付けたことがあるか | 受け付けたら true |

写っているコードを「未検出」として数えると、中心の判定が入れ替わるたびに「離した」と誤判定される。
そのため `misses` は**写っていないコードだけ**で増やす。

#### 6.4.3 受付の条件と定数

採用した1件が、次の条件を**すべて**満たすときに受け付ける。

1. `stableFrames >= STABLE_FRAMES`（中心に最も近い状態が続いている。一瞬の揺れで隣のコードを拾わない）
2. まだ受け付けたことがない（`everAccepted` が false）、または「離した」と判定済み（`released` が true）

| 定数 | 値 | 内容 |
|---|---|---|
| `STABLE_FRAMES` | 3 | 受け付けるまでに、連続して採用である必要があるフレーム数 |
| `RELEASE_MISSES` | 12 | 「離した」と判定するのに必要な、連続未検出フレーム数 |
| `RELEASE_MS` | 3000 | 「離した」と判定するのに必要な、最後の検出からの経過ミリ秒 |
| `FORGET_MISSES` | 30 | この回数だけ連続で検出されなければ、状態を削除する |

「離した」の判定は `misses >= RELEASE_MISSES` かつ 経過 `>= RELEASE_MS` の両方を満たしたときとする。
時間だけを条件にすると、手ぶれや照明でわずかに読み取りが途切れただけで解除されてしまうため、フレーム数も条件に加える。
...
stateDiagram-v2
    [*] --> 未知: 状態なし
    未知 --> 採用中: 検出・採用された
    採用中 --> 受付済: stableFrames ≧ 3（ここで購入リストに追加）
    採用中 --> 見えている: 別のコードが採用された（stableFrames = 0）
    受付済 --> 見えている: 別のコードが採用された
    受付済 --> 受付済: 同じコードを採用し続ける（再受付しない）
    見えている --> 採用中: ふたたび採用された
    見えている --> 離れた: 未検出（misses ＋1）
    受付済 --> 離れた: 未検出（misses ＋1）
    離れた --> 見えている: 再び検出された（misses = 0）
    離れた --> 離した: misses ≧ 12 かつ 経過 ≧ 3000ms
    離した --> 受付済: ふたたび採用され stableFrames ≧ 3（再受付）
    離した --> 未知: misses ≧ 30（状態を削除）
```

#### 6.4.5 運用上の注意と開発時の表示

- 同じ商品を複数個登録するときは、一度カメラから離してからかざし直す（`RELEASE_MISSES`・`RELEASE_MS` を満たす必要がある）。
  まとめて指定する場合は購入リストの数量変更を使う（FR-007）
- 読み取りの周期は 200ms。読めなかったフレームも1フレームとして数える。
  Barcode Detection API・ZXing のどちらの経路でも、読めなかったフレームを空として同じ判定関数に渡す
- 読み取り自体が例外になったフレーム（映像の準備前など）は、フレームとして数えない
- カメラの状態表示には、動作中の読み取り経路を「スキャン中（BarcodeDetector）」「スキャン中（ZXing）」のように表示する
- **開発環境（`APP_ENV=development`）のときだけ**、カメラ枠の下に判定の状態を表示する。
  経路、読み取りループの実行回数と1秒あたりの回数、このフレームの検出数と検出したコードの一覧、採用したコードと最後の検出からの経過、
  安定フレーム数、管理中のコード数、`misses`、受け付けた回数と直近の理由。
  あわせて、受け付けるたびに理由・離れていた時間・`misses`・安定フレーム数をブラウザのコンソールへ出力する。
  `APP_ENV` が未設定・`production` のときは、表示も出力も行わない（設定漏れは本番扱い）
- 実装：`frontend/src/lib/scanGate.ts` の純粋関数 `pickCenterMost`（採用の選択）と `nextScanState`（状態の更新と受付の判定）、
  および `BarcodeScanner`。両経路ともこの2つの関数を通る
...
### 実装中に AI と人間で切り分けた事項（2026-09-22、スキャンの重複受付）

**事象**：iPhone の画面にバーコードを表示してかざし続けると、同じ商品が繰り返し購入リストに追加された。

**進め方**：原因を推測で直さず、開発環境（`APP_ENV=development`）のときだけカメラ枠の下に判定の状態
（読み取り経路、読み取り回数と1秒あたりの回数、フレームごとの検出数と検出コード、採用したコード、安定フレーム数、
管理中のコード数、`misses`、受付回数と直近の理由）を表示し、受付のたびにコンソールへ出力するようにした。
人間がその数値を観察し、AI が仮説を立てて修正する、を繰り返した。

**仮説の更新（3段階）**

| 段階 | 観察した数値 | 分かったこと | 対応 |
|---|---|---|---|
| 1 | 読み取り 1002回に対して受付 193回（約1.25秒ごと） | 前回の受付からの経過時間だけで再受付していた | 「一度離した」と判定できるまで再受付しないことを必須にした |
| 2 | 検出数 2（1002 / 1004）、受付理由「別のコード」 | 1フレームに2つのバーコードが写り、採用が交互に入れ替わっていた | 映像の中心に最も近い1件だけを採用し、状態をコードごとに持つようにした |
| 3 | 管理中 1件（2つ写っているのに） | 採用されなかったが写っているコードを「見えていない」と数えていた | 「見えている」（`lastSeenAt`・`misses`）と「採用する」（受付の判定）を分離した |

さらに、中心の判定が一瞬揺れると隣のコードが受け付けられたため、3フレーム連続で採用されていることを受付の条件に加えた。

**AI の誤り**：段階3の原因は、AI（Claude）が作成した指示文の仕様（採用されなかったコードの `misses` を増やす）にあった。
実装はその指示どおりだったため、コードだけを見ても誤りに気づけない。診断表示の数値を人間が観察して初めて、
仕様そのものの誤りとして確認できた。**仕様を書いたのが AI であっても、検証は数値で行う必要がある。**

**人間が判断した事項**

- しきい値の採用（解除は 3000ms かつ 12フレーム、受付には3フレーム連続の採用）
- 複数写り込みの扱いを「複数写っている間は受け付けない」ではなく「中心に最も近い1件を採用し、安定条件を課す」方式にしたこと
  （早見表を使う運用では常に複数写るため、受け付けない方式では会計が進まない）

**確認環境**：Mac の Chrome（Barcode Detection API 経路）で確認した。
ZXing 経路（iPhone の Safari など）はカメラの利用に HTTPS が必要なため、Azure へデプロイしたあとに確認する。
